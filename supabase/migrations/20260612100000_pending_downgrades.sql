-- Migration to support tracking pending plan downgrades/upgrades.
-- Adds columns to profiles and updates admin_apply_subscription to store scheduled change data.

-- 1. Add pending plan columns to public.profiles if they don't exist
alter table public.profiles add column if not exists pending_plan text default null;
alter table public.profiles add column if not exists pending_plan_effective_at timestamp with time zone default null;

-- 2. Drop the old 5-parameter version of the function to prevent overloading issues
drop function if exists public.admin_apply_subscription(uuid, text, text, text, text);

-- 3. Create the new 7-parameter function version
...
-- 4. Apply permissions to the new function signature
revoke execute on function public.admin_apply_subscription(uuid, text, text, text, text, text, timestamp with time zone) from public;
grant execute on function public.admin_apply_subscription(uuid, text, text, text, text, text, timestamp with time zone) to service_role;
