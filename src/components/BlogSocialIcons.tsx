...
export const BLOG_CTA_SOCIAL_ORDER = ["LinkedIn", "Instagram", "Facebook", "Twitter", "YouTube", "Threads", "Pinterest", "Bluesky", "TikTok"];

export const BLOG_CTA_SOCIAL_BADGES: Record<string, { bg: string; icon: React.ReactNode }> = {
  LinkedIn: {
    bg: "bg-[#0077B5]",
...
export function BlogSocialIcons({ className }: { className?: string }) {
  return (
    <div className={cn("flex flex-row flex-wrap items-center justify-center gap-2.5 sm:gap-3 select-none", className)}>
      {BLOG_CTA_SOCIAL_ORDER.map((name) => {
        const badge = BLOG_CTA_SOCIAL_BADGES[name];
        ...
        return (
          <div
            ...
            title={name}
            className={cn(
              "w-9 h-9 rounded-none flex items-center justify-center border border-black/10 shadow-sm transition-transform duration-200 hover:scale-110 hover:-translate-y-0.5",
              badge.bg
            )}
          >
            {badge.icon}
