...
export type FreeToolFinalCtaProps = {
  onCtaClick?: () => void;
  headline?: string;
  subtitle?: string;
  className?: string;
...
export function FreeToolFinalCta({
  onCtaClick,
  headline = "Your content is ready. Your audience is waiting. ShipOS ships it.",
  subtitle = "Takes less than 5 minutes to connect your first platform and schedule your first post.",
  className,
}: FreeToolFinalCtaProps) {
  const navigate = useNavigate();
  const { resolvedTheme } = useTheme();
  const isDark = resolvedTheme === "dark";

  const handleClick = onCtaClick ?? (() => navigate("/signup"));

  return (
    <section className={cn("py-24 px-6 lg:px-8 bg-background relative z-10", className)}>
      <div className="max-w-6xl mx-auto relative">
        <div className="relative rounded-none bg-white dark:bg-[#1c1917] border-x-2 border-b-2 border-t-[8px] border-x-black border-b-black border-t-[#d75a34] dark:border-x-neutral-800 dark:border-b-neutral-800 p-10 md:p-16 text-center shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] dark:shadow-[8px_8px_0px_0px_rgba(215,90,52,0.15)] flex flex-col items-center">
          ...
          <div className="flex flex-row flex-nowrap items-center justify-center gap-2.5 sm:gap-6 mb-10 overflow-x-auto no-scrollbar w-full select-none py-1">
            {CTA_SOCIAL_BADGES.map((badge) => (
              <div key={badge.name} title={badge.name} className="transition-transform duration-200 hover:scale-110 shrink-0">
                <div
                  className={cn(
                    "w-8 h-8 sm:w-9 sm:h-9 rounded-none flex items-center justify-center border border-black/5 shadow-sm [&_svg]:w-4 [&_svg]:h-4 shrink-0",
                    badge.bg
                  )}
                >
                  {badge.icon}
          ...
          <Button onClick={handleClick} variant="marketing" size="lg" className="h-14 px-8 text-base tracking-wide group">
            Try it for $0 (7-days) →
          </Button>
        </div>
      </div>
