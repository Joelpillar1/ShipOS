...
export default function XEngagementCalculator() {
  ...
  const [avgImpressions, setAvgImpressions] = useState<string>("10000");
  const [likes, setLikes] = useState<string>("450");
  const [reposts, setReposts] = useState<string>("60");
  const [replies, setReplies] = useState<string>("25");
  const [bookmarks, setBookmarks] = useState<string>("30");
  ...
  const impressionsNum = Math.max(parseFloat(avgImpressions) || 0, 0);
  const likesNum = Math.max(parseFloat(likes) || 0, 0);
  const repostsNum = Math.max(parseFloat(reposts) || 0, 0);
  const repliesNum = Math.max(parseFloat(replies) || 0, 0);
  const bookmarksNum = Math.max(parseFloat(bookmarks) || 0, 0);

  // Calculations
  const totalInteractions = likesNum + repostsNum + repliesNum + bookmarksNum;
  const impressionEngagementRate = parseFloat(((totalInteractions / Math.max(impressionsNum, 1)) * 100).toFixed(2));
  const followerEngagementRate = parseFloat(((totalInteractions / followersNum) * 100).toFixed(2));

  // Determine tier & advice based on impression engagement
  let rating = "Good";
  let ratingColor = "text-[#d75a34]";
  let advice = "Healthy interaction velocity! To increase reposts and bookmarks, compile your frameworks into organized X threads, formatting key steps in clear bullet checklists.";
  
  if (impressionEngagementRate < 0.8) {
    rating = "Below Average";
    ratingColor = "text-amber-600";
    advice = "Your view-to-interaction conversion is low. Try adding a strong curiosity-inducing hook to stop feed scrollers, using short paragraphs (1-2 lines), and concluding with an easy call to action or reply prompt.";
  } else if (impressionEngagementRate >= 0.8 && impressionEngagementRate <= 2.2) {
    rating = "Average";
    ratingColor = "text-[#d75a34]";
    advice = "Healthy interaction velocity! To increase reposts and bookmarks, compile your frameworks into organized X threads, formatting key steps in clear bullet checklists.";
  } else if (impressionEngagementRate > 2.2 && impressionEngagementRate <= 4.5) {
    rating = "Good";
    ratingColor = "text-emerald-600";
    advice = "Excellent audience resonance! Your posts capture active bookmarks and reposts. Engage with top replies in the first 30 minutes of posting to stimulate conversation multipliers.";
  } else {
    rating = "Excellent";
    ratingColor = "text-violet-600 dark:text-violet-400";
    advice = "Premium X authority rank! Channels with ERs over 4.5% command high sponsorship CPMs. Focus on pitching executive brands and cross-posting visual slides.";
  }

  // Estimated Earnings calculator (sponsored post estimate)
  const sponsoredMin = Math.round(Math.max((impressionsNum / 1000) * 15, (followersNum / 10000) * 100));
  const sponsoredMax = Math.round(Math.max((impressionsNum / 1000) * 35, (followersNum / 10000) * 250));

  const handleCopyResults = () => {
    const textToCopy = `X / Twitter Audit:
Followers: ${followersNum.toLocaleString()}
Average Impressions: ${impressionsNum.toLocaleString()}
Impression-based Engagement Rate: ${impressionEngagementRate}% (${rating})
...
Calculated via ShipOS Premium Growth Suite`;
    ...
    toast({
      ...
      description: "X audit summary has been copied to your clipboard."
    });
  };
  ...
  const handleAutoSchedule = () => {
    const draftText = `Checking my X metrics! Est. Sponsored Post Value: $${sponsoredMin}-$${sponsoredMax} #XGrowth`;
  ...
  const faqs = [
    {
      q: "What is a good engagement rate on X (Twitter)?",
      a: "An impression-based engagement rate between 0.8% and 2.2% is average/healthy for X. Rates above 2.2% are considered good, and anything exceeding 4.5% is excellent, putting you in the top tier of viral creators."
    },
    {
      q: "How does X calculate engagement rate?",
      a: "X defines engagement rate as the number of engagements (clicks, retweets, replies, follows, likes, bookmarks) divided by impressions. This measures active interaction relative to organic views."
    },
    {
      q: "Why are bookmarks and reposts weighted so heavily by the X algorithm?",
      a: "Bookmarks signal to the recommendation engine that your content has high utility value to return to. Reposts amplify your reach directly to your followers' networks. Both metrics are prioritized over simple likes for organic distribution."
    },
    {
      q: "How can I increase my X engagement rate?",
      a: "You can increase your ER by writing short, scannable posts, adding structured bullet checklists, posting high-value threads, asking conversational questions, and pinning top replies to stimulate engagement loops."
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-[#FAF7F5] dark:bg-background">
      <SEO
        title="Free X / Twitter Engagement Rate Calculator & Value Auditor | ShipOS"
        description="Audit your X / Twitter account performance instantly. Calculate impression-based and follower-based engagement, and estimate sponsored post market values."
        path="/x-engagement-calculator"
        type="website"
        keywords={["x engagement calculator", "twitter engagement calculator", "sponsored post calculator", "x value auditor", "creator audit tool"]}
        jsonLd={[
          faqSchema(faqs),
          breadcrumbSchema([
            { name: "Home", path: "/" },
            { name: "Free Tools", path: "/x-engagement-calculator" },
            { name: "X Engagement Calculator", path: "/x-engagement-calculator" }
          ]),
      ...
      <main className="tools-header-padding pb-10">
        ...
        <section className="max-w-7xl mx-auto px-6 text-center mb-12">
          ...
          <h1 className="text-4xl sm:text-5xl font-black tracking-tight text-foreground mb-4">
            X (Twitter) Engagement Calculator
          </h1>
          <p className="text-base text-muted-foreground max-w-2xl mx-auto leading-relaxed">
            Audit your viral reach. Calculate exact post engagement statistics, compare your profile to industry baselines, and discover sponsored post market value ranges.
          </p>
          ...
          <div className="max-w-3xl mx-auto mt-8 p-6 bg-white dark:bg-[#1c1917] border-2 border-black dark:border-neutral-800 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(215,90,52,0.15)] text-left">
            <h2 className="text-sm font-black uppercase tracking-wider text-[#d75a34] mb-2 flex items-center gap-2">
              <Sparkles className="w-4 h-4 animate-pulse" /> Quick Answer: What is X Engagement Rate?
            </h2>
            <p className="text-sm text-foreground/90 leading-relaxed font-medium">
              An **X Engagement Rate** measures the level of interaction (likes, reposts, replies, and bookmarks) relative to impressions (views) or follower size. The standard impression-based formula is:
              ...
              While average benchmarks range between <strong>0.8% and 2.2%</strong>, rates over <strong>2.2%</strong> demonstrate solid organic distribution, and rates exceeding <strong>4.5%</strong> are considered outstanding for brand campaigns.
            </p>
          </div>
        </section>
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="border border-border bg-card p-6 md:p-8 rounded-none shadow-sm space-y-5">
              <h2 className="text-lg font-bold text-foreground tracking-tight border-b border-border pb-3 flex items-center gap-2">
                <svg className="w-5 h-5 text-foreground" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg> Profile Parameters
              </h2>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <label className="text-[10px] font-bold tracking-widest text-muted-foreground uppercase flex items-center gap-1.5 font-black">
                      <Eye className="w-3.5 h-3.5 text-[#d75a34]" /> Average Impressions
                    </label>
                    <span className="text-xs font-bold text-foreground bg-muted border border-border px-2 py-0.5 rounded-none">
                  ...
                  <Slider
                    value={[Math.log10(Math.max(impressionsNum, 10))]}
                    onValueChange={(val) => setAvgImpressions(String(roundToNiceNumber(Math.pow(10, val[0]))))}
                    min={2}
                    max={6.7}
                    step={0.01}
                    className="py-2"
                ...
                <div className="space-y-2">
                  ...
                  <Slider
                    ...
                    onValueChange={(val) => setLikes(String(roundToNiceNumber(Math.pow(10, val[0]))))}
                    min={0}
                    max={5}
                    step={0.01}
                    className="py-2"
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <label className="text-[10px] font-bold tracking-widest text-muted-foreground uppercase flex items-center gap-1.5 font-black">
                      <Share2 className="w-3.5 h-3.5 text-[#d75a34]" /> Average Reposts
                    </label>
                    <span className="text-xs font-bold text-foreground bg-muted border border-border px-2 py-0.5 rounded-none">
                      {repostsNum.toLocaleString()}
                    </span>
                  </div>
                  <Slider
                    value={[Math.log10(Math.max(repostsNum, 1))]}
                    onValueChange={(val) => setReposts(String(roundToNiceNumber(Math.pow(10, val[0]))))}
                    min={0}
                    max={4.3}
                    step={0.01}
                    className="py-2"
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <label className="text-[10px] font-bold tracking-widest text-muted-foreground uppercase flex items-center gap-1.5 font-black">
                      <MessageCircle className="w-3.5 h-3.5 text-[#d75a34]" /> Average Replies
                    </label>
                    <span className="text-xs font-bold text-foreground bg-muted border border-border px-2 py-0.5 rounded-none">
                      {repliesNum.toLocaleString()}
                    </span>
                  </div>
                  <Slider
                    value={[Math.log10(Math.max(repliesNum, 1))]}
                    onValueChange={(val) => setReplies(String(roundToNiceNumber(Math.pow(10, val[0]))))}
                    min={0}
                    max={4}
                    step={0.01}
                    className="py-2"
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <label className="text-[10px] font-bold tracking-widest text-muted-foreground uppercase flex items-center gap-1.5 font-black">
                      <Bookmark className="w-3.5 h-3.5 text-[#d75a34]" /> Average Bookmarks
                    </label>
                    <span className="text-xs font-bold text-foreground bg-muted border border-border px-2 py-0.5 rounded-none">
                      {bookmarksNum.toLocaleString()}
                    </span>
                  </div>
                  <Slider
                    value={[Math.log10(Math.max(bookmarksNum, 1))]}
                    onValueChange={(val) => setBookmarks(String(roundToNiceNumber(Math.pow(10, val[0]))))}
                    min={0}
                    max={4}
                    step={0.01}
                    className="py-2"
            ...
            <div className="border border-border bg-card p-6 md:p-8 rounded-none shadow-sm space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="border border-border bg-background p-6 rounded-none shadow-sm flex flex-col justify-center items-center text-center space-y-2">
                  <span className="text-[10px] font-bold tracking-widest text-muted-foreground uppercase">Impression Engagement Rate</span>
                  <div className={cn("text-4xl font-black tracking-tighter", ratingColor)}>
                    {impressionEngagementRate}%
                  </div>
                ...
                <div className="border border-border bg-background p-6 rounded-none shadow-sm flex flex-col justify-center items-center text-center space-y-2 md:col-span-2">
                  ...
                  <span className="text-[9px] text-muted-foreground font-bold tracking-wide">
                    Valuation based on standard sponsor CPM parameters ($15 - $35)
                  </span>
                </div>
              ...
              <div className="space-y-4 pt-2">
                <h3 className="text-xs font-bold text-foreground uppercase tracking-wider">
                  X Benchmark Comparison (Impression-Based)
                </h3>
                <div className="space-y-2.5">
                  {[
                    { label: "Your Engagement Rate", value: impressionEngagementRate, color: "bg-[#d75a34]" },
                    { label: "Average Industry Benchmark", value: 1.5, color: "bg-neutral-300 dark:bg-neutral-700" },
                    { label: "Premium Tier Target", value: 4.0, color: "bg-neutral-300 dark:bg-neutral-700" }
                  ].map((bar, i) => {
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground font-medium">
                    Auto-publish to X (Twitter), LinkedIn, Instagram, TikTok, and more.
                  </p>
                </div>
              </div>

              <p className="text-sm text-muted-foreground leading-relaxed">
                Consistency commands the X recommendation engine. Use ShipOS to auto-schedule your text hooks, compose viral thread blocks, format rich layout elements, and sync postings seamlessly.
              </p>
          ...
          <div className="max-w-5xl mx-auto px-6 lg:px-8">
            <div className="flex flex-col items-center text-center mb-16">
              ...
              <h2 className="text-3xl font-black text-foreground">
                How to Use the X Engagement Calculator
              </h2>
              <p className="text-sm text-muted-foreground mt-2 font-semibold tracking-wider">
                Audit and assess your tweet metrics in 3 simple steps.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="border border-border bg-card p-6 rounded-none shadow-sm space-y-4 text-center">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">
                  Drag the sliders to select your followers count, average impressions, average likes, reposts, replies, and bookmarks.
                </p>
              </div>
              ...
              <div className="border border-border bg-card p-6 rounded-none shadow-sm space-y-4 text-center">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">
                  The calculator instantly processes your parameters to display impression-based and follower-based engagement rates.
                </p>
              </div>
              ...
              <div className="border border-border bg-card p-6 rounded-none shadow-sm space-y-4 text-center">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">
                  Review the personalized algorithmic advice and connect your X profiles with ShipOS to begin scheduling your posts.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="max-w-5xl mx-auto px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="border border-border bg-card p-8 rounded-none shadow-sm space-y-4">
                <h3 className="text-xl sm:text-2xl font-black text-foreground">
                  Cracking the X Recommendation Engine
                </h3>
                <p className="text-sm text-muted-foreground leading-relaxed">
                  The X recommendation engine favors posts that drive active conversation. Bookmarks are heavily weighted because they indicate content utility. Reposts expand organic view distribution instantly, while likes have the lowest algorithmic weight.
                </p>
                <p className="text-sm text-muted-foreground leading-relaxed">
                  To hit recommendation algorithms, draft structured visual formatting patterns, reply to top comments early, and structure threads cleanly.
                </p>
              </div>
              <div className="border border-border bg-card p-8 rounded-none shadow-sm space-y-4">
                <h3 className="text-xl sm:text-2xl font-black text-foreground">
                ...
                <ul className="space-y-3 text-sm text-muted-foreground">
                  <li className="flex items-start gap-2.5">
                    ...
                    <span><strong>Hook Optimization:</strong> Compel scrollers to pause by using magnetic, clear curiosity triggers.</span>
                  </li>
                  <li className="flex items-start gap-2.5">
                    <span className="text-[#d75a34] font-bold text-base select-none leading-none">✓</span>
                    <span><strong>Thread Splits:</strong> Structure thread cards with scannable spaces. Place external links on the final slide card.</span>
                  </li>
                  <li className="flex items-start gap-2.5">
                    <span className="text-[#d75a34] font-bold text-base select-none leading-none">✓</span>
                    <span><strong>Bookmark Triggers:</strong> Include a CTA asking readers to 'save this thread for later review' to boost bookmarks.</span>
                  </li>
                </ul>
              </div>
            </div>
            ...
            <div className="mt-16 border-2 border-black dark:border-neutral-800 bg-white dark:bg-[#1c1917] p-6 md:p-8 rounded-none shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] dark:shadow-[6px_6px_0px_0px_rgba(215,90,52,0.15)]">
              <h3 className="text-xl font-black text-foreground mb-2 flex items-center gap-2">
                X Engagement Rate Benchmarks & Valuation Data
              </h3>
              <p className="text-sm text-muted-foreground mb-6 leading-relaxed">
                Cite this dataset when auditing profiles or preparing creator campaign pitches. Benchmark averages are calculated based on active impression volumes.
              </p>
                ...
                <table className="w-full border-collapse border border-border text-left text-xs sm:text-sm text-muted-foreground min-w-[600px]">
                  <thead>
                    <tr className="bg-muted text-foreground border-b border-border">
                      ...
                      <th className="p-3 font-bold border-r border-border">Avg. Impression Engagement Rate</th>
                      <th className="p-3 font-bold">Est. Sponsored Post Fee</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border">
                    <tr className="hover:bg-muted/30">
                      <td className="p-3 font-bold text-foreground border-r border-border">Nano Account</td>
                      <td className="p-3 border-r border-border">1,000 - 10,000 followers</td>
                      <td className="p-3 text-emerald-600 font-semibold border-r border-border">2.5% - 4.5%</td>
                      <td className="p-3 text-foreground font-semibold">$30 - $120 per post</td>
                    </tr>
                    <tr className="hover:bg-muted/30">
                      <td className="p-3 font-bold text-foreground border-r border-border">Micro Account</td>
                      <td className="p-3 border-r border-border">10,000 - 50,000 followers</td>
                      <td className="p-3 text-emerald-600 font-semibold border-r border-border">1.5% - 3.0%</td>
                      <td className="p-3 text-foreground font-semibold">$120 - $500 per post</td>
                    </tr>
                    <tr className="hover:bg-muted/30">
                      ...
                      <td className="p-3 text-emerald-600 font-semibold border-r border-border">1.0% - 2.0%</td>
                      <td className="p-3 text-foreground font-semibold">$500 - $2,000 per post</td>
                    </tr>
                    <tr className="hover:bg-muted/30">
                      <td className="p-3 font-bold text-foreground border-r border-border">Macro Account</td>
                      <td className="p-3 border-r border-border">500,000 - 2M+ followers</td>
                      <td className="p-3 text-emerald-600 font-semibold border-r border-border">0.6% - 1.2%</td>
                      <td className="p-3 text-foreground font-semibold">$2,000 - $8,000+ per post</td>
                    </tr>
                  </tbody>
                </table>
              </div>
        ...
        <section className="bg-white dark:bg-[#141413] border-t border-b border-border/40 py-20 relative z-10">
          <div className="max-w-4xl mx-auto px-6 lg:px-8 space-y-16">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm text-muted-foreground mt-2 font-semibold tracking-wider">
                Common questions about X impressions, bookmarks, algorithm metrics, and sponsorship valuations.
              </p>
            </div>
          ...
          <div className="max-w-5xl mx-auto px-6 lg:px-8">
            <div className="flex flex-col items-center text-center mb-14">
              ...
              <p className="text-base text-muted-foreground max-w-2xl leading-relaxed">
                Stop composing, start publishing. ShipOS auto-schedules your content across X (Twitter), LinkedIn, Instagram, TikTok, Threads, and Bluesky — all from one workspace.
              </p>
            </div>
