  ...
  const renderSortHeader = (label: string, column: 'views' | 'likes' | 'comments' | 'shares' | 'published') => {
    ...
    return (
      <button 
        ...
        className="flex items-center gap-1 hover:text-foreground font-bold transition-colors uppercase tracking-widest text-[10px]"
      >
        {label}
        {isActive ? (
          sortDirection === 'asc' ? (
      ...
      {!hasAccounts ? (
        <div className="space-y-6">
          ...
          <div className="flex flex-col justify-center items-center h-64 border border-dashed border-border p-8 text-center bg-card">
            ...
            <h3 className="text-sm font-black uppercase tracking-widest text-foreground">Analytics Unavailable</h3>
        ...
        <>
                ...
                <div className="flex items-center bg-card border border-border p-1 rounded-none shadow-sm h-9">
                  {['7d', '30d', '90d'].map((range) => (
                    <button
                      ...
                      className={cn(
                        "px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest transition-colors rounded-none h-full flex items-center justify-center",
                        timeFilter === range 
          ...
          {isRevalidating && (
            <div className="flex items-center gap-2 mb-4 text-[10px] font-bold uppercase tracking-widest text-muted-foreground animate-in fade-in">
                        ...
                        return (
                          <Card key={stat.label} className="rounded-none shadow-sm border-border bg-card flex flex-col justify-between overflow-hidden">
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1">
                              <CardTitle className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider leading-tight">{stat.label}</CardTitle>
                            ...
                            <div className="h-[350px] w-full relative">
                              {isFollowsUnsupported && !isFollowsNoticeDismissed && (
                                <div className="absolute inset-0 bg-background/80 backdrop-blur-[1.5px] flex flex-col items-center justify-center text-center p-6 z-10 border border-border/50">
                                  ...
                                  <h4 className="text-xs font-black uppercase tracking-wider text-foreground">Post-Level Growth Unavailable</h4>
                                  ...
                                  <button
                                    ...
                                    className="mt-4 px-3 py-1.5 bg-secondary text-secondary-foreground text-[10px] font-bold hover:bg-muted transition-colors rounded-none uppercase tracking-wider border border-border"
                                  >
                                    Dismiss Notice
                                  </button>
                                </div>
